...
from app.crud import create_conversion
...
async def download_media(request: DownloadRequest, user: dict = Depends(verify_token)):
    ...
    conversion_id = await create_conversion(
        user_id=user.get("id"),
        youtube_url=request.url,
        format=format_requested,
        youtube_video_name=""  # Placeholder, should be replaced with actual video name if available
    )
